...
drawbar(Monitor *m)
{
  ...
  // above was default, we patched so the bar is stable across monitors
  if (1) {
...
{
  // turns focus into keyboard only
  (void)e;
  // this used to make focus follow mouse
  // but we prefer to have focus entirely on keyboard
  // so we can do things like, typing on one monitor while
  // scrolling on the other one
  //
	// Client *c;
	// Monitor *m;
...
/* focus monitor and warp cursor to its center */
void
...
movemouse(const Arg *arg)
{
	...
	do {
		...
		case MotionNotify:
			...
			resize(c, nx, ny, c->w, c->h, 1);  // CHANGED: always move
			break;
		}
	...
	if (!wasfloating && selmon->lt[selmon->sellt]->arrange)
		arrange(selmon);
}
...
togglefloating(const Arg *arg)
{
  ...
  if (selmon->sel->isfloating) {
      int mx, my;
      getrootptr(&mx, &my);
      int fw = selmon->sel->w * 0.75;
      int fh = selmon->sel->h * 0.75;
      int fx = mx - fw / 2;
      int fy = my - fh / 2;
      resize(selmon->sel, fx, fy, fw, fh, 0);
  }
  arrange(selmon);
}
